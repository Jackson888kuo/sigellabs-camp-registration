...
開 issue 時務必加上對應優先級 label，便於排程、SLA 評估與通知策略。

| Label | 顏色 | 含義 | SLA | 適用情境 |
|---|---|---|---|---|
| **P0** | 🔴 深紅 (`#b60205`) | 緊急 | **24 小時內** | 線上家長無法報名/付款；確認信沒寄出；資料寫入失敗；Make scenario 完全當掉 |
| **P1** | 🟠 橘紅 (`#d93f0b`) | 高優先 | **3-5 天內** | 影響營運效率（如時區顯示錯誤）；可手動 workaround 但不便 |
| **P2** | 🟡 黃 (`#fbca04`) | 中高優先 | **本週內** | 改善類功能；非緊急但長期累積會拖累 |
| **P3** | 🟢 綠 (`#0e8a16`) | 中等優先 | **本月內** | 資料品質問題、邊角案例、追蹤表清理 |
| **P4** | 🔵 藍 (`#0366d6`) | 低優先 | **時間允許時** | nice-to-have、UI 微調、文件補充 |

**判斷流程：**
1. 是否有家長/外部使用者「現在」受影響？→ Y: P0；N: 繼續
2. 是否影響營運每日流程或無法手動處理？→ Y: P1；N: 繼續
3. 累積會擴大影響範圍嗎？→ Y: P2；N: 繼續
4. 屬於資料/品質類議題嗎？→ Y: P3；N: P4

> 💡 截止日明確的議題（如 Issue #1 早鳥 5/5 截止）應**標 P0** 並在 issue body 註明 deadline，不可僅用一般優先級判斷。
...
| 2026-04-28 | 新增第 8 章「優先級 Label 體系」（P0-P4 + SLA） | Jackson + Claude |
