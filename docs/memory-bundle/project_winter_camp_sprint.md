...
## Sprint Backlog（5 個 Issues）

| 順序 | # | 標題 | Label | 工作量 | 排程 |
|---|---|---|---|---|---|
| 1 | #6 | Iterator 勾 2 寫 6 bug（最優先） | bug / **P1** | 1.5 天 | 4/30–5/2 |
| 2 | #2 | 多營隊驗收測試 | bug / P2 | 0.5 天 | 5/2 |
| 3 | #1 | Module 10 payment_button_html 動態化 | bug / **P1** | 1 天 | 5/3 |
| 4 | #3 | 早鳥日期防呆 | enhancement | 0.5 天 | 5/4 |
| 5 | #8 | J 欄無表頭 + IML 未解析 | bug / P3 | 1 天 | 5/6–5/8 |
...
- 開發對話開始時，先讀 `docs/sprints/2026-W18-W20-winter-camp-prep.md` 與 `docs/issues/issue-1-implementation-spec.md`
- 順序嚴格：#6 → #2 → #1 → #3 → #8
...
| Git push to origin | ✅ Jackson 本機已執行 `git push origin main` |

## 2026-04-30 PM Cowork 進度（00:22-09:00 夜間 + 早晨產出）

> Jackson 因 4/29 整天耗損疲憊，於凌晨要求「找我可以做的事讓他先睡」。產出全部為「準備明早順利執行」的低風險文件 / 腳本，未動 Make scenario。早上 Jackson 醒來後完成 git commit + push、precheck 通過。
...
| 預檢一鍵腳本 | ✅ `scripts/issue-6-precheck.sh`（chmod +x）30 秒完成 §5.1 #2/#3/#4：snapshot 匯出 + Module 5 IML 比對 + 5→8 Filter 檢查 + 4 處 ref 路徑列印 | bash + python 語法已驗，5/1 早晨已執行通過 |
| 早晨啟動腳本 | ✅ `scripts/issue-6-morning-startup.sh` 處理 stale lock + commit + push + precheck 一鍵流程 | 已加強清理 .git/HEAD.lock 等 |
| 4/29 本機 snapshot 結構分析 | ✅ 確認 5 處目標位置精確 mapper path | 詳見「精確路徑表」|
...
| 30 分鐘操作卡 | ✅ `docs/issues/issue-6-v2-morning-operation-card.md`（9 章節、20+ checkbox、含預檢/Step 1-4/T1/緊急回退）| 明早照表逐步打勾 |
| Token 拖拉故障排除卡 | ✅ `docs/issues/issue-6-v2-token-drag-troubleshooting.md`（8 章節、5 大失敗徵兆、token 顏色辨識、4/28 方案 B 教訓回顧）| 拖拉當下對照判讀 |
| Cowork → Claude Code 交接文件 | ✅ `docs/handoff/2026-04-30-cowork-to-claude-code-issue-6.md`（11 章節含 Playwright vs Chrome MCP 風險分析）| Jackson 改用 Claude Code + Playwright MCP |
| Memory bundle 至 repo | ✅ `docs/memory-bundle/`（6 個關鍵 memory 副本 + INDEX.md）| 解決 Claude Code 看不到 Cowork memory 問題 |
| Memory 更新 | ✅ 本檔更新 + 「Module 11 lowercase x」事實已記錄 + feedback_workflow.md 加 Claude Code 升級段 | — |
| Git commit + push | ✅ Jackson 本機已 push（4/30 PM Cowork 產出 + 5/1 早晨）| — |
...
2. `bash scripts/issue-6-morning-startup.sh`（一鍵：清 lock + commit + push + precheck）
3. 開 VS Code → 啟動 Claude Code → 貼交接文件 §8 開場 prompt
4. Claude Code 讀完 4 份主文件 + 6 份 memory bundle 後回報計畫
5. 確認後執行操作卡 Step 1-4 + T1
6. 拖拉時一旦察覺異常 → 對照故障排除卡
7. T1 通過 → 立即 commit + push、撤銷 Make token
...
已填入 `活動設定表_Phase5_v5` Sheet 的 I/K 欄。詳見 `reference_phase5_sheets.md`（不在此 bundle，可從 Cowork 補匯出）。
